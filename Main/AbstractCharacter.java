...
public abstract class AbstractCharacter {
    ...
    private double drawTransmissionSiteFactor() {
        double draw_value;
        if (Class.sigma_trm == 0) {
            draw_value = Class.mu_trm;
        } else {
            draw_value = Class.randomProvider.nextGaussian(Class.randomProvider.indexMapping.get("topology"));
            draw_value = draw_value * Class.sigma_trm + Class.mu_trm; // transform into N(mu, sigma^2)
        }
        return draw_value;
    }
    ...
    public HashMap<Integer, HashSet<Integer>> evolveCharacter(boolean evolveWithReticulate) {
        ...
        Pair<HashMap<Integer, HashSet<Integer>>, Integer> res = evolveCharacterGenetically(Network.RootNode, stateSet, next_state, evolveWithReticulate);
        ...
        while (true) {
            ...
            for (Pair<Integer, Integer> edge : retEdges.keySet()) {
                ...
                handleReticulateEdge(retEdge, stateMap);
            ...
            for (Pair<Integer, Integer> edge : retEdges.keySet()) {
                ...
                // Do left
                Pair<HashMap<Integer, HashSet<Integer>>, Integer> hashMapIntegerPair = evolveCharacterGenetically(retEdge.left, stateMap.get(retEdge.left.Id), next_state, evolveWithReticulate);
                next_state = hashMapIntegerPair.snd;
                stateMap.putAll(hashMapIntegerPair.fst);

                // Do right
                Pair<HashMap<Integer, HashSet<Integer>>, Integer> hashMapIntegerPair2 = evolveCharacterGenetically(retEdge.right, stateMap.get(retEdge.right.Id), next_state, evolveWithReticulate);
    ...
    public abstract Pair<HashMap<Integer, HashSet<Integer>>, Integer> evolveCharacterGenetically(Vertex node, HashSet<Integer> current_state, int next_state, boolean stopAtReticulate);
    public abstract void handleReticulateEdge(ReticulateEdge retEdge, HashMap<Integer, HashSet<Integer>> stateMap);
